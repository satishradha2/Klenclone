...
from klen_clone.purchase_returns import OperationalPurchaseReturnReservation, OperationalPurchaseReturnWorkflowEvent, create_purchase_return, purchase_return_control_counts, rehearse_purchase_return_posting, replace_purchase_return, transition_purchase_return
...
def return_session(tmp_path):
    ...
    session.add(OperationalStockPosition(location_code="SHJ",sku="SKU-1",canonical_uom="piece",quantity_on_hand=Decimal("20"),quantity_reserved=Decimal("0"),average_unit_cost=Decimal("7"),availability_enabled=True,source_status="test_reconciled")); session.commit(); return session
...
def create(session,lines=None):
    return create_purchase_return(session,supplier_code="SUP-1",supplier_name_snapshot="Supplier One",location_code="SHJ",source_reference_type="goods_receipt",source_reference_key="GRN-1",return_date=date(2026,9,9),reason_code="quality",notes=None,actor="maker",lines=lines or [line()])
...
def test_edit_revision_and_locked_period_controls(tmp_path):
    session=return_session(tmp_path); document=create(session)
    document=replace_purchase_return(session,document,expected_revision=1,supplier_code="SUP-1",supplier_name_snapshot="Supplier One",location_code="SHJ",source_reference_type="goods_receipt",source_reference_key="GRN-2",return_date=date(2026,9,9),reason_code="wrong_item",notes="Reviewed",actor="maker",lines=[line(quantity="3",supplier="3",writeoff="0",reason=None)])
    assert document.revision==2 and document.source_reference_key=="GRN-2"
