...
def _normalise_line(raw: dict) -> dict:
    ...
    factor = Decimal(str(raw["factor_to_base_snapshot"])); price = Decimal(str(raw["unit_price"])); tax_rate = Decimal(str(raw["tax_rate"])); cost = Decimal(str(raw["unit_cost_snapshot"]))
    ...
    return dict(raw, quantity=quantity, restock_quantity=restock, writeoff_quantity=writeoff,
                ...
                unit_cost_snapshot=cost, disposition_reason=(raw.get("disposition_reason") or "").strip() or None)
...
def create_sales_return(session: Session, *, customer_code: str, customer_name_snapshot: str,
                        ...
                        reason_code: str, notes: str | None, actor: str, lines: list[dict]) -> OperationalSalesReturn:
...
def replace_sales_return(session: Session, document: OperationalSalesReturn, *, expected_revision: int,
                         ...
                         notes: str | None, actor: str, lines: list[dict]) -> OperationalSalesReturn:
...
def rehearse_sales_return_posting(session: Session, document: OperationalSalesReturn, *, actor: str) -> dict:
    if document.status != "approved" or not document.credit_note: raise ValueError("Only an approved sales return with a credit note can be rehearsed")
    period=session.scalar(select(OperationalFiscalPeriod).where(OperationalFiscalPeriod.starts_on<=document.return_date,
        OperationalFiscalPeriod.ends_on>=document.return_date,OperationalFiscalPeriod.status=="open",OperationalFiscalPeriod.rehearsal_enabled.is_(True)))
    if not period: raise ValueError("Return date is not in an open rehearsal-enabled fiscal period")
    restock_cost=sum(((line.restock_quantity*line.factor_to_base_snapshot*line.unit_cost_snapshot).quantize(MONEY,rounding=ROUND_HALF_UP) for line in document.lines),Decimal("0.00"))
    writeoff_cost=sum(((line.writeoff_quantity*line.factor_to_base_snapshot*line.unit_cost_snapshot).quantize(MONEY,rounding=ROUND_HALF_UP) for line in document.lines),Decimal("0.00"))
    total_cost=restock_cost+writeoff_cost
    journal=[{"account":"Sales Returns","debit":document.subtotal,"credit":Decimal("0.00")},
             {"account":"Output VAT","debit":document.tax_amount,"credit":Decimal("0.00")},
             {"account":"Accounts Receivable","debit":Decimal("0.00"),"credit":document.total_amount},
             {"account":"Inventory","debit":restock_cost,"credit":Decimal("0.00")},
             {"account":"Returned Goods Write-off","debit":writeoff_cost,"credit":Decimal("0.00")},
             {"account":"Cost of Goods Sold","debit":Decimal("0.00"),"credit":total_cost}]
    movements=[{"line_no":line.line_no,"location":document.location_code,"sku":line.sku,
                ...
                "value_delta":(line.restock_quantity*line.factor_to_base_snapshot*line.unit_cost_snapshot).quantize(MONEY,rounding=ROUND_HALF_UP)}
               for line in document.lines if line.restock_quantity>0]
    debit=sum((row["debit"] for row in journal),Decimal("0")); credit=sum((row["credit"] for row in journal),Decimal("0"))
    if debit != credit: raise RuntimeError("Sales return posting rehearsal is not balanced")
    source=json.dumps({"return_key":document.return_key,"revision":document.revision,"journal":journal,"movements":movements},default=str,sort_keys=True,separators=(",",":"))
    fingerprint=hashlib.sha256(source.encode()).hexdigest()
    session.add(OperationalAuditEvent(event_key=str(uuid.uuid4()),event_type="sales_return.posting_rehearsed",actor=actor,
        resource_key=document.return_key,detail=f"AED {debit}; {len(movements)} restock movements; fingerprint {fingerprint}; no posting"))
    session.commit()
    return {"return_key":document.return_key,"return_no":document.return_no,"credit_note_no":document.credit_note.credit_note_no,
            "posting_enabled":False,"period_key":period.period_key,"journal":journal,"movements":movements,
            "restock_cost":restock_cost,"writeoff_cost":writeoff_cost,"debit":debit,"credit":credit,
            "posting_fingerprint":fingerprint,"idempotency_key":f"sales-return:{document.return_key}:{document.revision}:{fingerprint[:20]}"}
